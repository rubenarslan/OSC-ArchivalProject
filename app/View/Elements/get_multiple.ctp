...
<div class="actions btn-group">
	...
	<ul class="dropdown-menu">
		...
		<li><?php echo $this->Html->link('View '.$user."'s coding", "/codedpaper/view/$id"); ?></li>
		<?php } ?>
	</ul>
</div>
<div class="actions btn-group">
	<a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#"><?php echo __('Compare…'); ?><span class="caret"></span>
	  </a>
	<ul class="dropdown-menu">
		<?php 
		for($i=0;$i < count($users);$i++) {
			for($j = $i; $j < count($users); $j++) {
				...
				$user2 = $users[$j];
				if($user != $user2) { ?>
		<li><?php echo $this->Html->link($user." <-> ".$user2, "/codedpaper/compare/$id/$id2"); ?></li>
		<?php }
			}
		} ?>
	</ul>
